...
#else
    const char* pszModule = "supermaxcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "supermaxcoin";
#else
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "supermaxcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "supermaxcoind.pid"));
