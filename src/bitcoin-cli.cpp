...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("supermaxcoin Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  supermaxcoin-cli [options] <command> [params]  " + _("Send command to supermaxcoin Core") + "\n" +
                  "  supermaxcoin-cli [options] help                " + _("List commands") + "\n" +
                  "  supermaxcoin-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
UniValue CallRPC(const string& strMethod, const UniValue& params)
{
    ...
    // supermaxcoin:
    // we get supermaxcoind output (possibly binary) of .write() that either:
