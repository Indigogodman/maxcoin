...
CBlockTemplate* CreateNewBlockInner(const CScript& scriptPubKeyIn, bool fAddProofOfStake, bool& fPoSCancel, CWallet* pwallet)
{
    ...
    if (fAddProofOfStake)  // attemp to find a coinstake
    {
        ...
        if (fPoSCancel)
            return NULL; // supermaxcoin: there is no point to continue if we failed to create coinstake
    }
    else
...

    {
        ...
        CValidationState state;
        if (!TestBlockValidity(state, *pblock, pindexPrev, false, false, false)) // supermaxcoin: we do not check block signature here, since we did not sign it yet
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("supermaxcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("supermaxcoinMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread(fProofOfStake? "supermaxcoin-stake-minter" : "supermaxcoin-miner");
    ...
    try {
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in supermaxcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("Running supermaxcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("supermaxcoinMiner terminated\n");
	return;
        // throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("supermaxcoinMiner runtime error: %s\n", e.what());
        return;
    }
}
