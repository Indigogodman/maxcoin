...
#if defined(NDEBUG)
# error "supermaxcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "supermaxcoin Signed Message:\n";
CHooks* hooks = InitHook(); //this adds namecoin hooks which allow splicing of code inside standart supermaxcoin functions.
...
{
    // supermaxcoin: return false if time is below block 10 000 - we have no PoS blocks below 10 000 in our official blockchain.
    ...
    // supermaxcoin: in our official blockchain we currently have max PoS == 37.4635 (for blocks 1..129646)
...
void ThreadScriptCheck() {
    RenameThread("supermaxcoin-scriptch");
...
{
    // supermaxcoin: moved from CheckBlock(), because this check now depends on context
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fWriteNames)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        else
        {
            ...
            // supermaxcoin: moved to here from CheckInputs(), because this check now depends on context
            if (tx.IsCoinStake())
            {
    ...
    // supermaxcoin: collect valid name tx
    ...
    // supermaxcoin: add names to nameindex.dat
    if (fWriteNames)
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (!Params().SkipProofOfWorkCheck())
    {
        ...
        {
            // this is needed only for supermaxcoin official blockchain, because of mistake we made at the beginning
...
// supermaxcoin: note, this function can process blocks without strict order
...
bool AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    // supermaxcoin: set PoS flag for CBlockIndex. This should probably be done immediately after we added header to CBlockIndex (i.e., after or inside AddToBlockIndex),
...
{
    // Preliminary checks
    //bool checked = CheckBlock(*pblock, state); // supermaxcoin: removed, since this check happens later in AcceptBlock function
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    // supermaxcoin: set/unset network serialization mode for new clients
