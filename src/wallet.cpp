                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl)
{
    ...
    // supermaxcoin: define some values used in case of namecoin tx creation
    ...
    txNew.nVersion = wtxNew.nVersion; // supermaxcoin: important for name transactions

    {
        LOCK2(cs_main, cs_wallet);
        {
            CAmount nMinOut = GetMinTxOut(chainActive.Tip()->GetBlockVersion(), chainActive.Tip()->pprev);
            nFeeRet = max(nFeeInput, MIN_TX_FEE);  // supermaxcoin: a good starting point, probably...
	    LogPrintf("CWallet::CreateTransactionInner: nMinOut=%lu nFeeRet=%lu\n", (unsigned long long)nMinOut, (unsigned long long)nFeeRet);
            while (true)
            {
                ...
                // supermaxcoin: in case of namecoin tx we have already supplied input.
		...
		// supermaxcoin: name tx always at first position
                if (!wtxNameIn.IsNull())
                {
                ...
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx*,unsigned int)& coin, setCoins)
                {
                    // supermaxcoin: we sign name tx differently.
