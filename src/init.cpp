...
void Shutdown()
{
    ...
    RenameThread("supermaxcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "supermaxcoin.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "supermaxcoind.pid") + "\n";
#endif
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2013-%i The supermaxcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("supermaxcoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("supermaxcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. supermaxcoin Core is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. supermaxcoin Core is probably already running."), strDataDir));
#ifndef WIN32
    ...
    // supermaxcoin: allow user to set alert and checkpoint pubkeys
    {
    ...
    bool fLoaded = false;
    int fAuxReindex = 0;   // supermaxcoin: used when upgrading pre-auxpow blockindex
    while (!fLoaded) {
        ...
        if (!fLoaded) {
            ...
            if (!fReset) {
                // supermaxcoin: try to reindex if no auxpow flag
                bool fAuxPow;
    ...
    // supermaxcoin: check in nameindex need to be created or recreated
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of supermaxcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart supermaxcoin Core to complete") << "\n";
